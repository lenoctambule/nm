                        ...
                        Elf64_Half e_shstrndx)
{
    ...
    if ((e_type > ET_CORE)
        ...
        return print_error(file->path, "Unsupported file."),(0);
    if (e_shoff + e_shentsize * e_shnum > (Elf64_Off)file->s.st_size)
        return print_error(file->path, "Invalid sections headers offset/size."),(0);
    if (e_phoff + e_phentsize * e_phnum > (Elf64_Off)file->s.st_size)
        return print_error(file->path, "Invalid programs headers offset/size."),(0);
    if (e_shstrndx > e_shnum)
        return print_error(file->path, "Invalid shstrndx."),(0);
    return (1);
}
